...
import type { ProviderResponse } from "../../datacite/types.js";

export function registerResource(server: McpServer): void {
  server.resource(
    "datacite-provider",
    new ResourceTemplate("datacite://provider/{id}", { list: undefined }),
    {
    ...
    async (uri, variables) => {
      ...
      const response = await getCached<ProviderResponse>(
        staticCache,
        `provider:${id}`,
        () => dataciteClient.get<ProviderResponse>(`/providers/${encodeURIComponent(id)}`)
      );
