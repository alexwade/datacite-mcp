...
export class DataCiteClient {
  ...
  private async fetchWithRetry(
    ...
    for (let attempt = 0; attempt < maxRetries; attempt++) {
      ...
      if (res.status === 429 || res.status >= 500) {
        ...
        lastError = new DataCiteError(
          `HTTP ${res.status} from DataCite API`,
          res.status
        );
  ...
  async getUrl<T>(targetUrl: string): Promise<T> {
  ...
  async getText(targetUrl: string, accept: string): Promise<string> {
    const res = await this.fetchWithRetry(targetUrl, {
      Accept: accept,
      "User-Agent": USER_AGENT,
    });
