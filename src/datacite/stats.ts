 ...
 */
export async function fetchClientDoiCount(clientId: string): Promise<number | null> {
  try {
    ...
    if (total === undefined) {
      console.error(
        `[datacite-mcp] includeStats: meta.total absent for client ${clientId}`
      );
      return null;
    }
    return total;
    ...
    console.error(
      `[datacite-mcp] includeStats: stats fetch failed for client ${clientId}:`,
