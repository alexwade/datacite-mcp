...
type ResolveResult =
  | { type: "found"; client: ResolvedClient }
  | { type: "ambiguous"; candidates: ClientCandidate[] }
  | { type: "not_found" };

// In-memory cache for the server's lifetime.
const nameCache = new Map<string, ResolvedClient | null>();
...
export async function resolveRepositoryName(
  ...
  const response = await dataciteClient.get<ClientsResponse>("/clients", {
  ...
  const exactMatches = data.filter(
    (r) => r.attributes.name.toLowerCase() === key
  );
  ...
  const withCounts: ClientCandidate[] = await Promise.all(
    pool.map(async (r) => {
      let doiCount = 0;
      try {
        const res = await dataciteClient.get<{ meta: { total: number } }>(
          "/dois",
          { "client-id": r.id, "page[size]": 0 }
        );
...
const ArgsSchema = {
  repository_name: z
    ...
    .min(1)
    .describe(
      "Common repository name (e.g. 'Zenodo', 'Dryad', 'arXiv'). Case-insensitive."
    ),
};
...
export function registerPrompt(server: McpServer): void {
  server.prompt(
    "repository-summary",
    "Generate a comprehensive metadata quality and activity report for a DataCite repository.",
    ArgsSchema,
    async (args) => {
      ...
      if (resolution.type === "ambiguous") {
        const rows = resolution.candidates
          .map(
            (c, i) =>
              `${i + 1}. **${c.name}** — client_id: \`${c.clientId}\` — DOIs: ${c.doiCount.toLocaleString()}`
          )
          .join("\n");
        return {
          messages: [
