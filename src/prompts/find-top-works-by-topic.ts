...
const ArgsSchema = {
  resource_type: z
    .string()
    .transform((val, ctx) => {
      const match = RESOURCE_TYPE_ENUM.find(
    ...
    .describe(
      "DataCite controlled resource type (e.g. Dataset, Software, Preprint). Case-insensitive."
    ),
  topic: z
    .string()
    .min(1)
    .describe(
      "Subject area or keywords to search (e.g. 'single-cell RNA sequencing', 'climate downscaling')."
    ),
};
...
export function registerPrompt(server: McpServer): void {
  server.prompt(
    ...
    "Find the top 10 most relevant works in DataCite for a given resource type and subject area.",
    ArgsSchema,
    (args) => {
      const resourceType = args.resource_type as string;

      const supplementaryNotes: Record<string, string> = {
        ComputationalNotebook: `
