...
const ArgsSchema = {
  identifier: z
    ...
    .min(1)
    .describe(
      "ORCID iD (e.g. 0000-0001-8135-3489, with or without https://orcid.org/ prefix) " +
...
export function registerPrompt(server: McpServer): void {
  server.prompt(
    "researcher-profile",
    "Generate a researcher profile showing all works registered with DataCite. " +
      "Provide an ORCID iD for exact matching, or a name for fuzzy search with disambiguation.",
    ArgsSchema,
    (args) => {
