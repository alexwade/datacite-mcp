...
export function registerTool(server: McpServer): void {
  server.tool(
    ...
    async (params) => {
      ...
      if (input.query)      apiParams["query"]       = input.query;
      if (input.clientType) apiParams["client-type"] = input.clientType;
      if (input.providerId) apiParams["provider-id"] = input.providerId;

      const cacheKey = `list-repos:${JSON.stringify(
      ...
      try {
        const response = await getCached<ClientsResponse>(
          staticCache,
          cacheKey,
          () => dataciteClient.get<ClientsResponse>("/clients", apiParams)
        );
        ...
        let doiCounts: (number | null)[] | undefined;
        if (input.includeStats && clients.length > 0) {
          doiCounts = await Promise.all(
            clients.map((r) => fetchClientDoiCount(r.id))
          );
        }
