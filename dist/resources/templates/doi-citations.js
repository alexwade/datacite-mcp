...
export function registerResource(server) {
    server.resource("datacite-doi-citations", new ResourceTemplate("datacite://doi/{doi}/citations", { list: undefined }), {
        ...
        const response = await getCached(searchCache, `citations:${doi}:25`, () => dataciteClient.get(`/dois/${encodeURIComponent(doi)}/citations`, {
            "page[size]": 25,
            detail: true,
        }));
