...
export function registerResource(server) {
    server.resource("datacite-doi-references", new ResourceTemplate("datacite://doi/{doi}/references", { list: undefined }), {
        ...
        const response = await getCached(searchCache, `references:${doi}:25`, () => dataciteClient.get(`/dois/${encodeURIComponent(doi)}/references`, {
            "page[size]": 25,
            detail: true,
        }));
            ...
            contents: [
                {
                    uri: uri.toString(),
                    mimeType: "application/json",
                    text: JSON.stringify({ doi, works, total: response.meta?.total ?? works.length }, null, 2),
                },
            ],
        };
    });
