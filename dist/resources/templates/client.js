...
export function registerResource(server) {
    server.resource("datacite-client", new ResourceTemplate("datacite://client/{id}", { list: undefined }), {
        description: "Full metadata for a specific DataCite repository client, including " +
        ...
        const [response, doiCount] = await Promise.all([
            getCached(staticCache, `client:${id}`, () => dataciteClient.get(`/clients/${encodeURIComponent(id)}`, { include: "prefixes" })),
            fetchClientDoiCount(id),
        ]);
        // Resource templates always show stats; null means the secondary call failed.
        const stats = { doiCount: doiCount ?? null };
