...
const GetRepositorySchema = z.object({
    id: z
        .string()
        .min(1)
        .describe("DataCite client ID (e.g. 'cern.zenodo', 'dryad.dryad'). " +
        "Use list_repositories to discover IDs."),
    includeStats: z
        .boolean()
        .default(false)
        .describe("When true, adds doiCount fetched via /dois?client-id={id}&page[size]=0. " +
...
export function registerTool(server) {
    server.tool("get_repository", "Retrieve full metadata for a single DataCite repository (client) by its ID. " +
        ...
        "Note: per-client view/download/citation counts are not exposed by the DataCite API.", GetRepositorySchema.shape, async (params) => {
        const input = GetRepositorySchema.parse(params);
        try {
            const [response, doiCount] = await Promise.all([
                getCached(staticCache, `client:${input.id}`, () => dataciteClient.get(`/clients/${encodeURIComponent(input.id)}`, { include: "prefixes" })),
                input.includeStats
            ...
            const stats = input.includeStats ? { doiCount: doiCount ?? null } : undefined;
